...
<hr><hr>
	<form action="" method="post" enctype="multipart/form-data" >
		...
		<table border="1">
			<tr>
				<td  width="100">ID</td>
				<td><input type="text" name="bo_u_id" value="${vo.u_id }" readonly="readonly"></td>
			</tr>
    		...
    		<tr>
     			...
     			<c:if test="${id=='admin' }" >
     				...
     				<option value="상품문의" selected="selected">상품문의
     			
     				</option>
       			</select>
     			<input type="text" name="bo_subject" size="30" id="subject" value="${vo.bo_subject }"></td>
    		</tr>
    		...
    		<tr>
     			...
     			<select name="bo_p_rating">
     				...
     				<option value=1
     				<c:if test="${vo.bo_p_rating ==1 }">
     				selected="selected"
     				</c:if>
     				>★☆☆☆☆</option>
     				<option value=2
     				<c:if test="${vo.bo_p_rating ==2 }">
     				selected="selected"
     				</c:if>
        			>★★☆☆☆</option>
     				<option value=3
     				<c:if test="${vo.bo_p_rating ==3 }">
     				selected="selected"
     				</c:if>
     				>★★★☆☆</option>
     				<option value=4
     				<c:if test="${vo.bo_p_rating ==4 }">
     				selected="selected"
     				</c:if>
     				>★★★★☆</option>
     				<option value=5
     				<c:if test="${vo.bo_p_rating ==5 }">
     				selected="selected"
     				</c:if>
     				>★★★★★</option>
       			</select>
    		...
    		<tr>
     			<td>Content</td>
     			<td><textarea name= "bo_content" id = "bo_content" rows ="30" cols="100" id="content" maxlength="200">${vo.bo_content }</textarea></td>
	    		<script>
				 var ckeditor_config = {
				   resize_enaleb : false,
				   enterMode : CKEDITOR.ENTER_BR,
				   shiftEnterMode : CKEDITOR.ENTER_P,
				   filebrowserUploadUrl : "../board/ckUploadModify"
				 };
    		...
    		<tr>
         		...
         		<input type="button" value="게시글 목록" onclick="location.href='../board/boardList';"></div>
     			</td>
    		</tr> 
		</table>
