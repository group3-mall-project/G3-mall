	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$("select").change(function(){
				...
				self.location = "./allUsersOrdersOutput?numOfItemsPerPage="+numOfItemsPerPage;
			});
		});
	</script>
</head>
<body>
	...
	<c:if test="${pvo.pre }">
		<a href="./allUsersOrdersOutput?numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }&currentPageNum=${pvo.beginNumOfPage-1 }">&laquo;</a>
	</c:if>
	<c:forEach begin="${pvo.beginNumOfPage }" end="${pvo.endNumOfPage }" var="i">
		<a href="./allUsersOrdersOutput?numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }&currentPageNum=${i }">${i }</a>
	</c:forEach>
	<c:if test="${pvo.nxt }">
		<a href="./allUsersOrdersOutput?numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }&currentPageNum=${pvo.endNumOfPage+1 }">&raquo;</a>
	</c:if>
</body>
</html>