	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$("select").change(function(){
				...
				var u_id = "${u_id }";
				self.location = "./singleUserOrdersOutput?numOfItemsPerPage="+numOfItemsPerPage+"&u_id="+u_id;
			});
		});
	</script>
</head>
<body>
	...
	<table border="1">
		...
		<c:forEach var="ovo" items="${listOfOrdersOfUser }">
			<tr>
				<td>
					<a href="./orderDetailOutput?o_d_num=${ovo.o_d_num }&backDest=${backDest}&currentPageNum=${pvo.cri.currentPageNum }&numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }&u_id=${u_id}">${ovo.o_d_num }</a>
				</td> 	
				<td>${ovo.o_p_name }</td> 	
				<td>${ovo.o_p_num }</td> 	
	...
	<c:if test="${pvo.pre }">
		<a href="./singleUserOrdersOutput?u_id=${u_id }&currentPageNum=${pvo.beginNumOfPage-1 }&numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }">&laquo;</a>
	</c:if>
	<c:forEach begin="${pvo.beginNumOfPage }" end="${pvo.endNumOfPage }" var="i">
		<a href="./singleUserOrdersOutput?u_id=${u_id }&currentPageNum=${i }&numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }">${i }</a>
	</c:forEach>
	<c:if test="${pvo.nxt }">
		<a href="./singleUserOrdersOutput?u_id=${u_id }&currentPageNum=${pvo.endNumOfPage+1 }&numOfItemsPerPage=${pvo.cri.numOfItemsPerPage }">&raquo;</a>
	</c:if>
</body>
</html>